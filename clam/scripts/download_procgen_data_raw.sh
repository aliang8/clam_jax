...
for i in "${!download_tasks[@]}"; do
    ...
    gdown --continue --output /scr/shared/prompt_dtla/datasets/procgen/expert_data/$task.zip $file_id
    unzip -q -o /scr/shared/prompt_dtla/datasets/procgen/expert_data/$task.zip -d /scr/shared/prompt_dtla/datasets/procgen/expert_data/ && rm /scr/shared/prompt_dtla/datasets/procgen/expert_data/$task.zip
done